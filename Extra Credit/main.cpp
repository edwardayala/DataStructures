...
int main ()
{
	...
	ifstream inData("graph1.dat");
//	inData.open("graph1.dat");
	if (inData.is_open())					// inData cannot open graph1.dat ?
		cout << "Data...\n";
	else
		cout << "no data\n";
...
bool findEulerCircuit(Graph * g, string start, string * circuit, int & num )
{
	...
	//...
	Stack st;
	Node * ecNode = new Node(start);
	st.push(new Node(start));
	while (!st.isEmpty())
	{
		if (ecNode->getNext())
		{
			Node * p = ecNode->getNext();
			st.push(p);
			g->addEdge(ecNode->getInfo(), p->getInfo());
		}
		else
		{
			circuit[num] = st.pop();
